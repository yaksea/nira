...
(function($) {
				...
				.droppable(
						{
							...
							onDragOver : function(e, _1b) {
								...
								$(_1b).draggable("proxy").removeClass(
							...
							onDragLeave : function(e, _1e) {
								$(_1e).draggable("proxy").removeClass(
							...
							onDrop : function(e, _1f) {
								...
								if ($(this).hasClass("tree-node-append")) {
									...
									_21 = _24;
									_22 = $(this).hasClass("tree-node-top") ? "top"
											: "bottom";
								}
								setTimeout(function() {
									_21(_1f, _20, _22);
			...
			if (ck.length) {
				...
				if (_41.onlyLeafCheck) {
					$("<span class=\"tree-checkbox tree-checkbox0\"></span>")
							.insertBefore(_42.find(".tree-title"));
					_c(_3f);
				}
			}
		} else {
	...
	function _48(_49, ul, _4a, _4b) {
		...
		var _4d = [];
		var _4e = $(ul).prev("div.tree-node").find(
		...
		function _4f(ul, _52, _53) {
			for ( var i = 0; i < _52.length; i++) {
				...
				$.data(_55[0], "tree-node", {
					id : _54.id,
					text : _54.text,
					iconCls : _54.iconCls,
					attributes : _54.attributes
				});
				$("<span class=\"tree-title\"></span>").html(_54.text)
						.appendTo(_55);
				if (_4c.checkbox) {
					if (_4c.onlyLeafCheck) {
						if (_54.state == "open"
								&& (!_54.children || !_54.children.length)) {
							if (_54.checked) {
								$(
				...
				if (_54.children && _54.children.length) {
					...
					if (_54.state == "open") {
						$(
								...
								.addClass(_54.iconCls).prependTo(_55);
	...
	function _57(_58, ul, _59) {
		...
		if (!_59) {
			_59 = true;
			$(_58).find("span.tree-indent").removeClass(
					"tree-line tree-join tree-joinbottom");
			$(_58).find("div.tree-node").removeClass(
		...
		var _5f = $(ul).children("li:last").children("div.tree-node").addClass(
				"tree-node-last");
		_5f.children("span.tree-join").removeClass("tree-join").addClass(
				"tree-joinbottom");
		...
		function _5d(_63) {
			...
			_63.next().find("div.tree-node").each(
					function() {
						$(this).children("span:eq(" + (_64 - 1) + ")")
								.addClass("tree-line");
					});
		}
		;
	...
	function _aa(_ab, _ac) {
		...
		if (ul.children("li").length == 0) {
			var _ae = ul.prev();
			_ae.find(".tree-icon").removeClass("tree-folder").addClass(
					"tree-file");
	...
	function _37(_bb, _bc) {
		var _bd = $.extend({}, $.data(_bc, "tree-node"), {
			target : _bc,
			checked : $(_bc).find(".tree-checkbox").hasClass("tree-checkbox1")
		});
		if (!_43(_bb, _bc)) {
			_bd.state = $(_bc).find(".tree-hit").hasClass("tree-expanded") ? "open"
					: "closed";
		}
		return _bd;
	}
	...
	function _c9(_ca, _cb) {
		...
		_cf.width(_ce + 20);
		_cf.height(document.compatMode == "CSS1Compat" ? (18 - (_cf
				.outerHeight() - _cf.height())) : 18);
	...
	$.fn.tree = function(_db, _dc) {
		...
		return this.each(function() {
			...
			if (_dd) {
				...
				_de = $.extend({}, $.fn.tree.defaults, $.fn.tree
						.parseOptions(this), _db);
				$.data(this, "tree", {
					options : _de,
					tree : _1(this)
				});
