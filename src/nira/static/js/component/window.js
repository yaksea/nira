...
(function($){
	...
	function hideTip(target){
		var valid=$(".validatebox-text", target);
		valid.each(function(){
			$(this).validatebox("hideTip");
		});
	}
	/**
	...
	function init(target, options){
		...
		if (state){
			...
			var t = $(target);
			opts = $.extend({}, $.fn.window.defaults, $.fn.window.parseOptions(target), options);
			$(target).attr('title', '');
...
//		console.info(state.options.left)
//		console.info(state.options.top)
...
//		if (state.options.top == null){
			...
			var winTop=($(window).height() - height) / 2 + $(document).scrollTop();
			state.options.top = winTop>0?winTop:0;
//		}
//		console.info(state.options.left)
//		console.info(state.options.top)
		win.window('move');
