...
(function($){
	function createBox(target){
		...
		if (!state.spinner){
			var p = $('<div style="padding:2px"><span style="width:170px" /></div>').insertAfter(panel.children('div.datebox-calendar-inner'));
			state.spinner = p.children('span');
	...
	function getCurrentDate(target){
		...
		var s=t.timespinner('getSeconds');
		var millisec=h*60*60*1000+m*60*1000+s*1000;
		//根据输入时间 调整  
		var temp=new Date(date.getFullYear(), date.getMonth(), date.getDate(),0,0,0);
		return new Date(temp.getTime()+millisec);
	}
	...
	$.fn.datetimebox.defaults = $.extend({}, $.fn.datebox.defaults, {
		...
		formatTime:"hh:mm:ss",
		onSelect:function(date){
			$(this).datetimebox('calendar').calendar('options').current=date;
		},
		keyHandler: {
			up:function(){},
			down:function(){},
		...
		formatter:function(date){
			...
			return $.fn.datebox.defaults.formatter(date) + ' ' +this.formatDate(formatNumber(h),formatNumber(M),formatNumber(s));
		},
		parser:function(s){
			if ($.trim(s) == ''){
				return new Date();
		...
		formatDate:function(h,m,s){
			var temp=this.formatTime;
			temp=temp.replace("hh",h);
			temp=temp.replace("mm",m);
			temp=temp.replace("ss",s);
			return temp;
		}
	});
})(jQuery);
