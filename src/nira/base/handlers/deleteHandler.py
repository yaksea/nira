...
class DeleteRequestHandler(EditRequestHandler):    
#    数据输出处理
    def __init__(self, *args, **kwargs):
        super(DeleteRequestHandler, self).__init__(*args, **kwargs)
    
    @authenticate    
    def post(self):
        id = self.params['id']
        mongo.db[self.module.name].update({'_id':id},{'$set':{'isDeleted':True, 'version':utility.getVersion()}})
        self.sendMsg()
